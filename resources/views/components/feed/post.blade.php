{{--
    ...
    A post is a white card on the feed's gray ground. The separation between
    posts is the gap, not a rule — so there is no border here, and the spacing
    belongs to the list rather than to the card.
...
>
    ...
    <div class="flex items-center gap-2.5">
        <a :href="post.store.url || post.url"
           class="h-10 w-10 shrink-0 overflow-hidden rounded-full">
            ...
            <template x-if="! post.store.logo">
                <span class="flex h-full w-full items-center justify-center rounded-full bg-green-100 text-[13px] font-semibold text-green-700"
        ...
        <div class="min-w-0 flex-1">
            <a :href="post.url"
               class="block truncate text-[15px] font-medium leading-tight text-gray-900"
               x-text="post.name"></a>
            ...
            <template x-if="post.store.url">
                <a :href="post.store.url"
                   class="mt-0.5 block truncate text-xs leading-tight text-gray-500 underline-offset-2 active:underline"
                   x-text="post.store.location ? post.store.name + ' · ' + post.store.location : post.store.name"></a>
            </template>
            <template x-if="! post.store.url">
                <p class="mt-0.5 truncate text-xs leading-tight text-gray-500"
                   x-text="post.store.location ? post.store.name + ' · ' + post.store.location : post.store.name"></p>
            </template>
        </div>
    </div>

    {{-- 2. CAPTION — above the image, the way a post reads. Clamped to about two
         lines by character count rather than line-clamp, because the "…more"
         has to sit inline at the cut and a CSS clamp gives nothing to hang it on. --}}
    <template x-if="post.caption">
        <p class="mt-2.5 text-sm leading-relaxed text-gray-600">
            ...
            --><button type="button" x-show="! expanded && post.caption.length > 90" @click="expanded = true"
                      class="ml-1 text-gray-400">…more</button>
        </p>
    </template>

    {{-- 3. IMAGE. Lazy and async so a fast scroll never blocks on decoding an
         image the reader has already passed. --}}
    <a :href="post.url" class="mt-2.5 block overflow-hidden rounded-xl bg-brand-bg">
        <img
            :src="post.image.src"
    ...
    <div class="mt-3 flex items-center gap-4">
        <button type="button" @click="$store.feed.toggleLike(post)" class="flex items-center gap-1.5 text-gray-500" :aria-pressed="post.liked">
            <svg class="h-[21px] w-[21px] transition-transform active:scale-90" :class="post.liked ? 'text-brand' : 'text-gray-500'"
            ...
            <span class="text-[13px] text-gray-500" x-text="post.like_count" x-show="post.like_count > 0"></span>
        </button>

        <button type="button" @click="$store.feed.save(post)" aria-label="Save">
            <svg class="h-[21px] w-[21px] text-gray-500 transition-transform active:scale-90"
        ...
        <button type="button" @click="$store.feed.share(post)" aria-label="Share">
            <svg class="h-[21px] w-[21px] text-gray-500 transition-transform active:scale-90" fill="none" stroke="currentColor" stroke-width="1.8" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" d="M7.217 10.907a2.25 2.25 0 1 0 0 2.186m0-2.186c.18.324.283.696.283 1.093s-.103.77-.283 1.093m0-2.186 9.566-5.314m-9.566 7.5 9.566 5.314m0 0a2.25 2.25 0 1 0 3.935 2.186 2.25 2.25 0 0 0-3.935-2.186Zm0-12.814a2.25 2.25 0 1 0 3.933-2.185 2.25 2.25 0 0 0-3.933 2.185Z" />
            </svg>
        </button>

        <div class="ml-auto flex items-center gap-2.5">
            <p class="text-[15px] font-medium text-brand">
                ₦<span x-text="Number(post.price).toLocaleString()"></span>
            </p>
            <button type="button" @click="$store.feed.buyNow(post)"
                    class="rounded-full bg-brand px-4 py-2 text-sm font-medium text-white transition-transform active:scale-95">
                Buy Now
            </button>
        </div>
    </div>
