    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div class="lg:col-span-8 space-y-6">
            <form method="POST" action="{{ route('procurement.storeFinancials') }}" id="financialsForm">
                ...
                {{-- Payment Method --}}
                <div class="bg-white dark:bg-zinc-800 rounded-xl p-6 border border-[#becab5]/30 dark:border-zinc-700 shadow-[0px_4px_20px_rgba(0,0,0,0.04)]">
                    <h2 class="text-base font-semibold text-[#191c1d] dark:text-zinc-100 border-b border-[#e1e3e4] dark:border-zinc-700 pb-3 mb-5"
                        style="font-family:'Montserrat',sans-serif;">Payment Information</h2>

                    <div class="mb-5">
                        <label class="text-[10px] font-bold text-[#6f7b68] dark:text-zinc-500 uppercase tracking-wider block mb-3">Payment Method</label>
                        <div class="grid grid-cols-3 gap-4">
                            @foreach([['bank_transfer','account_balance','Bank Transfer'],['cash','payments','Cash'],['credit','credit_card','Credit']] as [$val,$icon,$lbl])
                            <label class="flex flex-col items-center justify-center p-4 border border-[#becab5] dark:border-zinc-600 rounded-lg cursor-pointer hover:bg-[#f3f4f5] dark:hover:bg-zinc-700 transition-colors
                                has-[:checked]:border-[#016c00] has-[:checked]:bg-green-50 dark:has-[:checked]:bg-green-900/20 has-[:checked]:text-[#016c00] dark:has-[:checked]:text-green-400 text-[#6f7b68] dark:text-zinc-400">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label class="text-[10px] font-bold text-[#6f7b68] dark:text-zinc-500 uppercase tracking-wider block mb-2">Amount Paid (₦)</label>
                            <div class="relative">
                                <span class="absolute left-3 top-1/2 -translate-y-1/2 text-[#6f7b68] dark:text-zinc-400 font-bold">₦</span>
                                <input type="number" name="amount_paid" id="amountPaid"
                                    ...
                                    class="w-full pl-8 pr-4 py-2.5 border border-[#becab5] dark:border-zinc-600 rounded-lg text-sm font-bold focus:border-[#016c00] focus:ring-2 focus:ring-[#016c00]/20 outline-none bg-white dark:bg-zinc-900 dark:text-zinc-100"
                                    required>
                            </div>
                            @error('amount_paid')
                                <p class="text-red-600 dark:text-red-400 text-xs mt-1">{{ $message }}</p>
                        ...
                        <div>
                            <label class="text-[10px] font-bold text-[#6f7b68] dark:text-zinc-500 uppercase tracking-wider block mb-2">Reference Number / Note</label>
                            <input type="text" name="reference_number"
                                value="{{ old('reference_number', $financials['reference_number'] ?? '') }}"
                                placeholder="e.g. TRF-2024-09-01"
                                class="w-full px-4 py-2.5 border border-[#becab5] dark:border-zinc-600 rounded-lg text-sm focus:border-[#016c00] focus:ring-2 focus:ring-[#016c00]/20 outline-none bg-white dark:bg-zinc-900 dark:text-zinc-100">
                        </div>
                    </div>
                </div>
        ...
        <div class="lg:col-span-4 sticky top-24">
            <div class="bg-white dark:bg-zinc-800 rounded-xl border border-[#becab5]/30 dark:border-zinc-700 shadow-[0px_4px_20px_rgba(0,0,0,0.04)] overflow-hidden">
                <div class="px-5 py-4 border-b border-[#e1e3e4] dark:border-zinc-700">
                    <h2 class="text-base font-semibold text-[#191c1d] dark:text-zinc-100" style="font-family:'Montserrat',sans-serif;">Order Summary</h2>
                </div>
                <div class="p-5 space-y-3">
                    ...
                    <div class="flex justify-between items-start">
                        <div>
                            ...
                            <p class="text-xs text-[#6f7b68] dark:text-zinc-400">Qty: {{ $item['quantity'] }}</p>
                        </div>
                        <span class="text-sm font-bold text-[#191c1d] dark:text-zinc-100" style="font-family:'Montserrat',sans-serif;">
                    ...
                    <div class="border-t border-[#e1e3e4] dark:border-zinc-700 pt-3 space-y-2">
                ...
                <div class="px-5 py-4 flex justify-between items-center border-t border-[#e1e3e4] dark:border-zinc-700">
                    <span class="text-base font-semibold text-[#191c1d] dark:text-zinc-100" style="font-family:'Montserrat',sans-serif;">Total Order Value</span>
                    <span class="text-xl font-bold text-[#016c00] dark:text-green-400" style="font-family:'Montserrat',sans-serif;">₦{{ number_format($subtotal, 2) }}</span>
                </div>
            </div>
